  ...
  const handleSave = async (data: any) => {
    try {
      if (view === "products") {
        ...
        if (selectedCategory) await productsService.updateCategory(selectedCategory.id, data);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        ...
        <div className="flex justify-end">
          {!showInactive ? (
            <Button
              onClick={() => { setSelectedProduct(null); setSelectedCategory(null); setIsModalOpen(true); }}
            ...
            >
              <Plus size={20} /> {view === 'products' ? 'Nuevo Producto' : 'Nueva Categoría'}
            </Button>
          ) : (
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden shadow-2xl">
        ...
        <Table
          ...
          emptyLabel={view === 'products' ? "No se encontraron productos" : "No se encontraron categorías"}
        >
          {view === 'products' 
      ...
      <ConfirmModal
        ...
        title={view === 'products' ? "¿DESACTIVAR PRODUCTO?" : "¿DESACTIVAR CATEGORÍA?"}
