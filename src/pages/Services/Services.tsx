...
import { Search, Edit2, Trash2, Plus, Layers, RotateCcw } from "lucide-react";
  ...
  const handleSave = async (data: any) => {
    try {
      if (view === "services") {
        ...
        if (selectedCategory)
          await servicesService.updateCategory(selectedCategory.id, data);
        ...
        <div className="flex justify-end">
          {!showInactive ? (
            ...
            <PermissionGate allowedRoles={[1]}>
              ...
              >
                <Plus size={20} />{" "}
                {view === "services" ? "Nuevo servicio" : "Nueva categoría"}
              </Button>
            </PermissionGate>
          ) : (
